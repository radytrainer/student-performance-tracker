  ...
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600">Monitor and resolve student performance alerts</p>
      </div>
      <div class="flex items-center space-x-3">
        ...
        <button @click="generate" class="px-4 py-2 bg-purple-600 text-white rounded">Generate</button>
      </div>
    </div>
        ...
        <tbody>
          <tr v-for="a in alerts" :key="a.id" class="border-t">
            ...
            <td class="px-4 py-2 text-sm text-right">
              <button v-if="!a.is_resolved" @click="resolve(a)" class="text-green-600 hover:text-green-800">Resolve</button>
...
const loadAlerts = async () => {
  ...
  const res = await alertsAPI.list(params, 'admin')
  const payload = res.data?.data
  alerts.value = payload?.data || payload || []
}

const resolve = async (a) => {
  await alertsAPI.update(a.id, { is_resolved: true }, 'admin')
  await loadAlerts()
}
...
const generate = async () => { await alertsAPI.generate(filters.value.term_id || null); await loadAlerts() }

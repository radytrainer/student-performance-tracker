  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-4 sm:py-8">
    <div class="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-4xl font-bold text-gray-900 mb-2">My Profile</h1>
      ...
      <div class="bg-white rounded-xl sm:rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="relative bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 px-4 sm:px-8 py-8 sm:py-12">
          <div class="flex flex-col items-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-8">
            ...
            <div class="relative group">
              <div class="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden bg-white shadow-2xl border-4 border-white/20 backdrop-blur-sm">
                ...
                <div v-else class="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                  ...
                  <span v-if="user.name" class="text-2xl sm:text-3xl font-bold text-gray-600">
            ...
            <div class="text-center sm:text-left text-white flex-1">
              <h2 class="text-xl sm:text-3xl font-bold mb-2">{{ user.name || 'Your Name' }}</h2>
              <p class="text-blue-100 text-base sm:text-lg mb-3">{{ user.email || 'your.email@example.com' }}</p>
              <div class="flex flex-wrap gap-2 justify-center sm:justify-start">
                ...
                <span class="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium bg-green-500/20 backdrop-blur-sm text-white border border-green-400/30">
        ...
        <div class="p-4 sm:p-8">
          <form @submit.prevent="saveProfile" class="space-y-6 sm:space-y-8" novalidate>
            <!-- Form Grid -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
              <!-- Personal Information Section -->
              <div class="space-y-4 sm:space-y-6">
                <div class="flex items-center space-x-2 sm:space-x-3 pb-3 sm:pb-4 border-b border-gray-200">
                ...
                <div class="space-y-4 sm:space-y-5">
                  <div>
                    ...
                    <p v-if="errors.name" class="text-red-500 text-xs sm:text-sm mt-1 flex items-center">
                  ...
                  <div>
                    ...
                    <p v-if="errors.email" class="text-red-500 text-xs sm:text-sm mt-1 flex items-center">
                      <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                      </svg>
                      {{ errors.email }}
                  ...
                  <div>
                    ...
                    <p v-if="errors.phone" class="text-red-500 text-xs sm:text-sm mt-1 flex items-center">
                      <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                      </svg>
                      {{ errors.phone }}
              ...
              <div class="space-y-4 sm:space-y-6">
                ...
                <div class="space-y-4 sm:space-y-5">
                  <div>
                    ...
                    <div class="relative">
                      <input
                        ...
                        disabled
                      />
                      <svg class="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <label for="memberSince" class="block text-sm font-semibold text-gray-700 mb-2">Member Since</label>
                    <div class="relative">
                      <input
                        id="memberSince"
                  ...
                  <div>
                    <label for="lastLogin" class="block text-sm font-semibold text-gray-700 mb-2">Last Login</label>
                    <div class="relative">
                      <input
                        id="lastLogin"
                  ...
                  <!-- Profile Completion -->
                  <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Profile Completion</label>
                    <div class="bg-gray-50 rounded-lg sm:rounded-xl p-3 sm:p-4">
                      <div class="flex items-center justify-between mb-2">
                        <span class="text-xs sm:text-sm font-medium text-gray-700">{{ profileCompletion }}% Complete</span>
                        <span class="text-xs sm:text-sm text-gray-500">{{ getCompletionStatus() }}</span>
                      </div>
            ...
            <!-- Action Buttons -->
            <div class="flex flex-col space-y-3 sm:flex-row sm:justify-end sm:space-y-0 sm:space-x-4 pt-6 sm:pt-8 border-t border-gray-200">
              <button
                v-if="!editing"
                type="button"
                @click="startEditing"
              ...
              <template v-else>
                ...
                >
                  <svg v-if="loading" class="animate-spin -ml-1 mr-2 h-4 w-4 sm:h-5 sm:w-5 text-white" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
    ...
    >
      <div v-if="notification.show" :class="getNotificationClasses()" class="fixed top-4 right-4 left-4 sm:left-auto sm:right-4 p-4 rounded-xl shadow-2xl z-50 max-w-sm mx-auto sm:mx-0">
        <div class="flex items-center">
          <div class="flex-shrink-0">
            <svg v-if="notification.type === 'success'" class="w-5 h-5 sm:w-6 sm:h-6 text-green-600" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
            </svg>
            <svg v-else class="w-5 h-5 sm:w-6 sm:h-6 text-red-600" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
            </svg>
          </div>
          ...
          <button @click="notification.show = false" class="ml-4 text-gray-400 hover:text-gray-600 touch-manipulation">
...
const isFormValid = computed(() => {
  return user.name && user.email && !Object.values(errors).some(error => error)
})

const profileCompletion = computed(() => {
  const fields = [user.name, user.email, user.phone, user.bio, profileImage.value]
...
const getInputClasses = (field) => {
  const baseClasses = 'w-full pl-8 sm:pl-10 pr-4 py-2.5 sm:py-3 border rounded-lg sm:rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 font-medium text-sm sm:text-base'
  const enabledClasses = 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 bg-white'
...
const getTextareaClasses = () => {
  const baseClasses = 'w-full px-4 py-2.5 sm:py-3 border rounded-lg sm:rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 font-medium resize-none text-sm sm:text-base'
  const enabledClasses = 'border-gray-300 focus:border-blue-500 focus:ring-blue-500 bg-white'
  const disabledClasses = 'border-gray-200 bg-gray-50 text-gray-600'
...
onMounted(async () => {
  await nextTick()
  // Initialize with sample data - in a real app, you'd load from an API
  originalUser.value = { ...user, profileImage: profileImage.value }
})
</script>
